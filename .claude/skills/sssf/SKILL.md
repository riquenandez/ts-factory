...
8. **A known command is code, not an agent** — if you can write the invocation down (`bun test`, `ruff check`), it belongs in a `kind="code"` phase via `adw_modules/quality.ts`. Agents are for the parts that need reading and deciding; failures come back to the builder as an envelope either way.
...
Both coding-agent interfaces are v1: `coding_agent: pi` and `coding_agent: claude_code`. Default model `gemini-3.6-flash` via openrouter, thinking `medium`. The visualizer app lives at `apps/visualizer/` and is started with `just obs`; sqlite queries work without it.
